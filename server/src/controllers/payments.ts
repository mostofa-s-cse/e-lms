...
export const createCustomPayment = async (req: Request, res: Response) => {
  try {
    ...
    const enrollmentData: any = {
      studentId: userId,
      courseId,
      status: 'PENDING', // All enrollments require admin approval
    };
    if (intakeId !== null) {
...
export const createCartPayment = async (req: Request, res: Response) => {
  try {
    ...
    for (const item of items) {
      ...
      // Get course info to determine if it's free
      const course = await prisma.course.findUnique({
        where: { id: item.courseId }
      });
      
      // Get intake info if provided
      let intake = null;
      if (intakeId) {
      ...
      // Determine if this is a free course/intake
