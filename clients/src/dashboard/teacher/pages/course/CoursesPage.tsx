...
const CoursesPage = () => {
  ...
  useEffect(() => {
    console.log('CoursesPage: User info:', user);
    console.log('CoursesPage: User role:', user?.role);
    console.log('CoursesPage: Token exists:', !!localStorage.getItem('token'));
    fetchCourses();
  }, [user]);
  ...
  const fetchCourses = async () => {
    try {
      setLoading(true);
      console.log('CoursesPage: Fetching courses for teacher...');
      const response = await coursesAPI.getByTeacher();
      console.log('CoursesPage: Courses response:', response);
      setCourses((response.data as CoursesResponse).data);
    } catch (error: any) {
      console.error('CoursesPage: Error fetching courses:', error);
      console.error('CoursesPage: Error response:', error.response?.data);
      console.error('CoursesPage: Error status:', error.response?.status);
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      ...
      // Automatically assign the current teacher
      if (user?.id) {
        submitData.append('teacherId', user.id);
      }
  ...
  const columns = [
    {
      ...
      render: (thumbnail: string | null, course: Course) => (
        <Link to={`/teacher/courses/${course.id}`} className="cursor-pointer">
