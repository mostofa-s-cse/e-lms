    ...
    try {
      ...
      if (amount === 0) {
        const enrollmentPayload = { 
          courseId: course.id,
          ...(selectedIntake ? { intakeId: selectedIntake } : {})
        };
        const response = await enrollmentsAPI.create(enrollmentPayload);

        if ((response.data as any).success) {
          const message = (response.data as any).message || 'Enrollment request submitted successfully! Your enrollment is pending admin approval.';
          showSuccessAlert('Request Submitted', message);
          setShowEnrollmentModal(false);
          // Redirect to dashboard after successful enrollment request
          navigate('/student/courses');
        } else {
